...
- `read_entries()` is removed. Use `tape.query.all()` for entry queries or `tape.read(TapeQuery().after_offset(...))` for offset reads.
...
- `append(TapeEntry)` is removed from tape sessions. Use `tape.append(payload, ...)`, `tape.handoff(...)`, or LLM calls that record their own entries.
...
- `TapeStream` and `AsyncTapeStream` are removed. `TapeStore` is the durable streamable boundary; `TapeManager` and `Tape` expose scoped operations directly.
