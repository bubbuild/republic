...
- `read_entries()` is removed. Use `tape.query.all()` for entry queries or `tape.query.after_offset(...).view()` for offset reads.
...
- `append(TapeEntry)` remains the tape-session write path. Schemaless records use explicit factories such as `TapeEntry.record(...)`.
...
- `TapeStream` and `AsyncTapeStream` are removed. `TapeStore` is the durable streamable boundary; `TapeManager` and `Tape` keep the existing scoped orchestration shape.
- `TapeStreamQuery` is removed. Use `TapeQuery().after_offset(...)`, `TapeQuery().include_anchors()`, and `TapeQuery().stop_at_close(...)` for offset reads.
